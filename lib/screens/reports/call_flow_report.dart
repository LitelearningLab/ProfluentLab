...
class CallFlowReport extends StatefulWidget {
  const CallFlowReport({key});

  @override
  State<CallFlowReport> createState() => _CallFlowReportState();
}

class _CallFlowReportState extends State<CallFlowReport> {
  ...
  Future<void> _getReports() async {
    ...
    var report = await db.getCallFlowReports(userDatas.appUser!.id!);
    ...
    for (int i = 0; i < _reports.length; i++) {
      print("last scoree function calledd");
      print("check:${_reports[_reports.keys.toList()[i]]!.sentences.first.lastScore}");
  ...
  Widget build(BuildContext context) {
    return BackgroundWidget(
      appBar: CommonAppBar(title: "Call Flow Practice Report"),
      body: _isLoading
          ? Center(
                    ...
                    Expanded(
                      child: ListView.builder(
                          padding: EdgeInsets.only(
                              top: isSplitScreen ? getFullWidgetHeight(height: 16) : getWidgetHeight(height: 16),
                              bottom: isSplitScreen ? getFullWidgetHeight(height: 60) : getWidgetHeight(height: 60),
                          ...
                          itemBuilder: (context, index) {
                            print("attempts checkkk:${_reports[_reports.keys.toList()[index]]?.attempts}");
                            return _reports[_reports.keys.toList()[index]]?.attempts == 0
                                ? SizedBox()
                                : InkWell(
                                    onTap: () {
                                      Navigator.push(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Container(
                                        height: isSplitScreen
                                            ? getFullWidgetHeight(height: 114)
                                            : getWidgetHeight(height: 114),
                                        decoration: BoxDecoration(
                                                          ...
                                                          Column(
                                                            ...
                                                            children: [
                                                                  ...
                                                                  children: [
                                                                    Text(
                                                                      ...
                                                                      style: TextStyle(
                                                                        ...
                                                                        fontSize:
                                                                            globalFontSize(kText.scale(11), context),
                                                                      ),
                                                                    ),
                                                                    Text(
                                                              ...
                                                              LinearPercentIndicator(
                                                                ...
                                                                percent:
                                                                    _reports[_reports.keys.toList()[index]]!.lastScore /
                                                                        100,
...
class CallFlowReport extends StatefulWidget {
  CallFlowReport({Key? key}) : super(key: key);

  @override
  _CallFlowReportState createState() {
    return _CallFlowReportState();
  }
}
...
class _CallFlowReportState extends State<CallFlowReport> {
  ...
  Future<void> _getReports() async {
    ...
    AuthState userDatas = Provider.of<AuthState>(context, listen: false);
    var report = await db.getCallFlowReports(userDatas.appUser!.id!);
    if (report.isNotEmpty) report.sort((a, b) => DateTime.parse(b.lastAttempt!).compareTo(DateTime.parse(a.lastAttempt!)));
    ...
    for (int i = 0; i < _reports.length; i++) {
      _reports[_reports.keys.toList()[i]]?.sentences.sort((a, b) => DateTime.parse(b.lastAttempt!).compareTo(DateTime.parse(a.lastAttempt!)));
      _reports[_reports.keys.toList()[i]]?.lastScore = _reports[_reports.keys.toList()[i]]!.sentences.first.lastScore!;
    }
    setState(() {
      _isLoading = false;
    ...
    return BackgroundWidget(
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text(
              "CALL FLOW PRACTICE REPORT",
              style: TextStyle(fontFamily: Keys.fontFamily, fontSize: 17, color: Colors.white, fontWeight: FontWeight.w500),
            ),
            Text(
              "CLICK ON THE SENTENCE FOR DETAILED REPORT",
              style: TextStyle(fontFamily: Keys.fontFamily, fontSize: 10, color: Colors.white, fontWeight: FontWeight.w500),
            ),
          ],
        ),
      ),
